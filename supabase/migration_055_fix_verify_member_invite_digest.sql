-- Migration 055: Fix invite verification digest lookup across extension schemas
-- Supabase commonly installs pgcrypto in `extensions`, while the previous RPC
-- used `set search_path = public`, which can fail with:
...
declare
  ...
  input_code_hash_alt text;
  input_code_hash_compact text;
...
begin
  ...
  input_code_hash := encode(digest(dashed, 'sha256'::text), 'hex');
  input_code_hash_alt := encode(digest(normalized, 'sha256'::text), 'hex');
  input_code_hash_compact := encode(digest(compact, 'sha256'::text), 'hex');

  select * into invite
  from member_invites mi
  where mi.code_hash in (input_code_hash, input_code_hash_alt, input_code_hash_compact)
     or mi.code_hash = normalized
     or replace(mi.code_hash, '-', '') = compact
  ...
  if invite.id is null and length(compact) >= 8 then
    ...
    select * into invite
    from member_invites mi
    where mi.code_prefix = prefix
    order by mi.created_at desc
    limit 1;

    if invite.id is not null then
      if invite.code_hash not in (input_code_hash, input_code_hash_alt, input_code_hash_compact, normalized)
